...
class visibility_of_element_located(object):
    ...
    用于检查某一个元素是否存在于页面的DOM上并且是可见的。
    可见性意味着不仅显示元素，而且元素的高度和宽度也大于0。

    可见时返回该元素对象，不可见时返回False
...
def _element_if_visible(element, visibility=True):
    '''判断元素对象是否对用户是可见的，可见时返回该元素，否则返回False'''
...
class visibility_of_all_elements_located(object):
    ...
    but also has a height and width that is greater than 0.
    用于检查所有元素是否出现在页面的DOM上且可见的期望。
    可见性意味着不仅显示元素，而且元素的高度和宽度也大于0。

    locator - used to find the elements
    returns the list of WebElements once they are located and visible
        一旦它们被定位并可见，返回WebElements的列表
    """
    def __init__(self, locator):
        self.locator = locator
...
class new_window_is_opened(object):
    ...
    windows handles increase
        将打开新窗口并增加窗口句柄数量的期望，即判断新窗口是否被打开。返回bool类型的结果
    """
